...
export function SolutionDialog({ href, puzzle }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <input type="hidden" name="moves" value={JSON.stringify(state.moves)} />
      ...
      <p>
        Not satisfied?<br />
        <a href={getResetHref(href.value)} className="underline text-purple-2">
          Start over
        </a>{" "}
        <em>or</em>{" "}
        <form method="dialog" className="inline">
          <button className="underline text-purple-2">close</button>
        </form>
      </p>
    </dialog>
  );
